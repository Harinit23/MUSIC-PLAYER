from tkinter import Tk, HORIZONTAL, Label, Button, END, PhotoImage, Listbox, LEFT, StringVar, Menu, Toplevel, ACTIVE, \
    BOTH, TOP, filedialog, messagebox, Entry
...
class Player(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        # =====additional buttons and entry======
        self.add_button = Button(master, text="Add Song", command=self.add_song)
        self.add_button.place(x=700, y=470)

        self.delete_button = Button(master, text="Delete Selected Song", command=self.delete_selected_song)
        self.delete_button.place(x=800, y=470)

        self.song_var = StringVar()
        self.song_entry = Entry(master, textvariable=self.song_var)
        self.song_entry.place(x=600, y=470)
    ...
    def append_listbox(self):
        ...
        # =====run the append_listbox function on a separate thread====== #
    ...
    def add_song(self):
        ...
        if song:
            self.play_list.insert(END, song)
            self.song_var.set('')
        else:
            messagebox.showinfo("Error", "Please enter a valid song name.")

    def delete_selected_song(self):
        selected_song = self.play_list.curselection()
        if selected_song:
            self.play_list.delete(selected_song)
        else:
            messagebox.showinfo("Error", "Please select a song to delete.")
    ...
    def main_window(self):
        ...
        self.button_load_music = Button(self.master,
                                        text='\U0001F4C1' + ' ♫ open folder with your music ♫ ',
        ...
        self.progress = ttk.Progressbar(self.lab, orient=HORIZONTAL,
